...
SELECT payment_id,amount,payment_date
FROM payment
WHERE payment_date >= DATE('2005-05-27 00:00:00') AND payment_date <= DATE('2005-05-28 00:00:00');
...
SELECT 
    ...
    description,
    release_year,
    rental_duration * rental_duration as total_rental_cost
FROM film
...
WHERE amount>5 LIMIT 1000 OFFSET 1000;
...
# from the staff table, performing 2 left joins with the address table 
# then the city table to get the address and city related columns.
SELECT
    staff.first_name,staff.last_name,
    address.address,address.address2,address.district,city.city,address.postal_code
FROM staff
LEFT JOIN 
